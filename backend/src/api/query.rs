...
// curl -v http://localhost:8080/api/v1/query -H 'Content-Type: application/json' -H 'tenant_id: 1' -d '{ "view": "comparable_card", "params": { "start": "20250101", "end": "20250201", "category_level1": "1" }, "spec": { "sql": "SELECT IF(s.shop_name = '"''"', '"'self'"', s.shop_name) merchant_name, SUM(COLLAPSE(s.transaction_amount, 0)) transaction_amount FROM default_datasource.dwd_rival_stats_distincted_di_1d s WHERE s.date_str BETWEEN {start} AND {end} AND s.category_level1 = {category_level1} <shop_names:AND s.shop_name IN {shop_names}> GROUP BY s.shop_name" } }'
// curl -v http://localhost:8080/api/v1/query -H 'Content-Type: application/json' -H 'tenant_id: 1' -d '{ "view": "comparable_card", "params": { "start": "20250101", "end": "20250201", "category_level1": "1", "shop_names": [ "1", "2", "3" ] }, "spec": { "sql": "SELECT IF(s.shop_name = '"''"', '"'self'"', s.shop_name) merchant_name, SUM(COLLAPSE(s.transaction_amount, 0)) transaction_amount FROM default_datasource.dwd_rival_stats_distincted_di_1d s WHERE s.date_str BETWEEN {start} AND {end} AND s.category_level1 = {category_level1} <shop_names:AND s.shop_name IN {shop_names}> GROUP BY s.shop_name" } }'
